  ...
  return (
    <>
      <div className="p-4 sm:p-6 rounded-3xl bg-white dark:bg-[#151620] border border-[#D8D8CF] dark:border-[#272730] shadow-subtle-depth space-y-4 relative overflow-hidden select-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 relative z-10">
          {top3Targets.map((target, idx) => {
            ...
            return (
              <div
                key={target.id}
                className={`p-3.5 sm:p-4 rounded-2xl border transition-all duration-200 relative flex flex-col justify-between space-y-3 group ${
                  target.completed
                    ? 'bg-emerald-500/[0.05] dark:bg-emerald-500/[0.08] border-emerald-500/35 shadow-[0_2px_12px_rgba(16,185,129,0.08)]'
                    : hasText
                    ? `bg-[#FAF9F5]/70 dark:bg-[#1C1D27] border-[#D8D8CF] dark:border-[#2E3044] shadow-xs ${meta.accentGlow}`
                    : 'bg-[#FAF9F5]/40 dark:bg-[#151620] border-dashed border-[#D0D0C5] dark:border-[#2A2B3D] hover:border-amber-500/40'
                }`}
              >
                {/* Slot Header */}
                  ...
                  {hasText && (
                    <div className="flex items-center gap-1 opacity-80 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="p-1 rounded-lg text-[#85877E] hover:text-[#11120F] dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/10 transition-all cursor-pointer"
                      ...
                      <button
                        ...
                        className="p-1 rounded-lg text-[#85877E] hover:text-rose-500 hover:bg-rose-500/10 transition-all cursor-pointer"
                        title="Clear slot"
                      >
                  ...
                  {isEditing ? (
                    <div className="flex items-center gap-1.5">
                      <input
                        ...
                        className="flex-1 px-3 py-1.5 rounded-xl bg-white dark:bg-[#12131C] border border-amber-500 text-xs font-bold text-[#11120F] dark:text-white focus:outline-none shadow-xs"
                      />
                      <button
                        onClick={() => handleSaveEdit(target.id)}
                        className="p-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-bold cursor-pointer transition-all shadow-xs active:scale-95"
                      >
                        <Check className="w-3.5 h-3.5 stroke-[3]" />
                      </button>
                      <button
                        onClick={() => setEditingTargetId(null)}
                        className="p-1.5 rounded-xl bg-[#EEEEE8] dark:bg-[#28293D] text-[#85877E] hover:text-[#11120F] dark:hover:text-white cursor-pointer"
                      >
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {target.subjectName && (
                          <div className="inline-flex items-center gap-1.5 mt-1.5 px-2 py-0.5 rounded-md bg-[#EEEEE8] dark:bg-[#242533] border border-[#D8D8CF]/80 dark:border-[#323448] text-[11px] font-mono font-bold text-[#191A17] dark:text-[#CBD5E1]">
                            <span
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-[#EEEEE8] dark:bg-[#222332] hover:bg-[#E2E2D8] dark:hover:bg-[#2B2C3E] text-[#11120F] dark:text-[#E2E4F0] text-xs font-bold transition-all cursor-pointer active:scale-95 shadow-2xs"
                        >
                        ...
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl bg-amber-500/10 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 hover:bg-amber-500/20 border border-amber-500/30 text-xs font-bold transition-all cursor-pointer active:scale-95"
                        >
                      ...
                      {isPicker && (
                        <div className="p-2.5 rounded-2xl bg-white dark:bg-[#12131C] border border-[#D8D8CF] dark:border-[#28293D] shadow-xl space-y-2 mt-1 z-20 animate-fade-in">
                          <input
                            type="text"
                            placeholder="Search syllabus topics..."
                            value={pickerSearch}
                            onChange={e => setPickerSearch(e.target.value)}
                            className="w-full px-3 py-1.5 rounded-xl bg-[#F7F6F0] dark:bg-[#1A1B28] border border-[#D8D8CF] dark:border-[#28293D] text-xs font-medium text-[#11120F] dark:text-white placeholder-[#85877E] focus:outline-none focus:border-amber-500"
                          />
                          <div className="max-h-44 overflow-y-auto space-y-1 custom-scrollbar">
                            {filteredTopics.map(t => (
                              ...
                              >
                                ...
                                <span
                                  className="text-[10px] font-mono font-bold px-2 py-0.5 rounded text-white shrink-0 ml-1.5"
                ...
                <div className="pt-2.5 border-t border-[#EEEEE8] dark:border-[#242533] flex items-center justify-between text-xs font-mono">
                  ...
                  {target.completed ? (
                    <span className="inline-flex items-center gap-1 text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/15 px-2 py-0.5 rounded-lg">
                      <span>✓</span>
                      <span>Crushed & Done</span>
                    </span>
                  ) : hasText ? (
                    <span className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-600 dark:text-amber-400 bg-amber-500/15 px-2 py-0.5 rounded-lg">
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
                      <span>In Progress</span>
                    </span>
                  ) : (
                    <span className="text-[11px] text-[#85877E] dark:text-[#686B80] italic">
                      Slot Empty
                    </span>
                  )}
                </div>
        ...
        <div className="p-3.5 rounded-2xl bg-[#FAF9F5] dark:bg-[#1A1B26] border border-[#D8D8CF] dark:border-[#272730] flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <span className="text-[11px] font-mono font-bold text-[#65675F] dark:text-[#A1A1B2] uppercase tracking-wider shrink-0">
              Sprint Velocity:
            </span>
            <div className="flex-1 sm:w-40 h-2.5 rounded-full bg-[#E2E2D8] dark:bg-[#282A3A] overflow-hidden p-0.5">
              <div
