...
import { Search, Sun, Moon, Flame, Zap, ChevronDown, Volume2, VolumeX, ExternalLink } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ onOpenSearch, onOpenSettings }) => {
  ...
  return (
    <header className="sticky top-0 z-30 h-16 w-full bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800/80 px-2 sm:px-6 md:px-8 flex items-center justify-between gap-1.5 sm:gap-2">
      ...
      {/* Right Controls - Mobile Optimized & Always Visible */}
      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
        ...
        >
          ...
          />
          <span className="hidden sm:inline">Mock Tracker</span>
        ...
        <button
          onClick={onOpenSearch}
          className="lg:hidden p-1.5 sm:p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 shrink-0"
        ...
        {/* XP (Desktop only) */}
        <div className="hidden md:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-brand-500/10 border border-brand-500/30 text-brand-600 dark:text-brand-400 shrink-0">
        ...
        <button
          ...
          className="hidden md:block p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 shrink-0"
        >
        ...
        {/* Dark Mode Toggle (Always Visible on Mobile & Desktop) */}
        <button
          onClick={toggleTheme}
          title={isDark ? 'Switch to Light Mode' : 'Switch to Dark Mode'}
          className="p-1.5 sm:p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors shrink-0"
        >
          {isDark ? (
        ...
        <button
          ...
          className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-brand-500 to-purple-600 flex items-center justify-center text-white text-[11px] sm:text-xs font-bold shadow-sm hover:scale-105 active:scale-95 transition-transform shrink-0"
        >
